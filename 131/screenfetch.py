...
def sysinfo_scrape(output):
   ...
   attributes = {}
   # Just interested in non-blank lines
   lines = [line for line in output.split('\n') if line]
   # Assume the first line gives the system name
   m = re.match('^.*\s\s([^\s]*)$', lines[0])
   attributes['Name'] = m.group(1).strip()
   # For all other lines we match "name: value" at the end of the line
   for line in lines[1:]:
      m = re.match('^.*\s\s([^:]*):(.*)$', line)
      if m:
