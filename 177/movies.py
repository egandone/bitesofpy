...
def group_by_genre(data=movie_excel_file):
    ...
    """
    movies_df = pd.read_excel("https://bit.ly/2BVUyrO", skiprows=7, usecols=range(2,4))
    movies_df.genres = movies_df.genres.str.split('|')
    movies_df = explode(movies_df, ['genres'])
    movies_df = movies_df.loc[movies_df.genres != '(no genres listed)']
    genre_counts = movies_df.groupby(by='genres').count().sort_values('movie', ascending=False)
    return genre_counts
