...
def create_parser():
    ...
    group.add_argument('-d', '--delete', help='delete a product by name (str)')
    group.add_argument('-l', '--list', help='show items in cart', action='store_true')
    group.add_argument('-s', '--search', help='search items by name')
    return parser
...
def handle_args(args=None, cart=None):
    ...
    if (args.list):
        cart.show()
    if (args.add):
        new_item = Item(product=args.add[0], price=int(args.add[1]), craving=eval(args.add[2]))
        cart.add(new_item)
        cart.show()
    if (args.delete):
...
if __name__ == '__main__':
    # Same starting cart as used in tests for so it is possible
    # just to try some samples from the command line.
    ...
    handle_args(cart=Groceries(items))