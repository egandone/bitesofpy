...
def process_data(url: str) -> pd.DataFrame:
    ...
    """
    data = get_data(url)
    return pd.read_csv(data["download_url"], parse_dates=[0])
...
def summary_report(df: pd.DataFrame, stats: Union[List[str], None] = STATS) -> None:
    ...
    years.sort()
    for year in years:
        year_stats = df[df['month'].dt.year == year].sales.describe()
        year_stats['sum'] = df[df['month'].dt.year == year].sales.sum()
        year_stats['median'] = year_stats['50%']
        row = {stat: year_stats[stat] for stat in stats}
        rows.append(row)
    stats_df = pd.DataFrame(rows, index=years)
...
def yearly_report(df: pd.DataFrame, year: int) -> None:
    ...
    Returns:
        ...
        12     69545.62
    """
    year_data = df[df['month'].dt.year == year].copy()
    if len(year_data) == 0:
        raise ValueError(f'The year {year} is not included in the report!')
    year_data['month'] = year_data['month'].dt.month
