...
def operas_both_at_premiere(guest, composer):
    ...
    if guest not in composers:
        raise ValueError(f'{guest} not a valid guest')
    if composer not in composers:
        raise ValueError(f'{composer} not in valid composer')
    ...
    for opera in [_o for _o in operas if _o.author == composer]:
        opening_date = _get_date(opera.date)
        # The opening date must be during the lifetime
        # of the guest and before the death of the composer
        if (opening_date > guest_born) and (opening_date < guest_death) and (opening_date < composer_death):
            possible_operas.append(opera.play)
        
    return possible_operas
